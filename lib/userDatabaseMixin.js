...
const UserDatabaseMixin = {
  ...
  async setPermissionProfiles(permissions) {
    if (!_.isObject(permissions)) {
      throw new Error('permissions must be an object', { cause: 'invalid' });
    }
    return this.knex.transaction(async (trx) => {
      ...
      inserts = await trx('MenuPermissions').insert(allPermissions).returning('idPermissionProfile');
  ...
  async checkUserIsAllowed(userId, resource, method) {
    ...
    const queryResult = await this.knex.select(
      ...
      .andWhere('Users.ExpiredAfter', '>', now)
