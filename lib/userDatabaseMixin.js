...
const UserDatabaseMixin = {
  ...
  async setPermissionProfiles(permissions) {
    ...
    return this.knex.transaction(async (trx) => {
      ...
      let inserts = await trx('Permission').insert(allPermissions).returning('idPermissionProfile');
      console.log(`Inserted ${inserts.length} Permissions`);

      inserts = await trx('PermissionProfiles').insert(allPermissionProfiles).returning('idPermissionProfile');
      console.log(`Inserted ${inserts.length} PermissionProfiles`);
    });
  },

  async setPermissionProfileAssignmentsForRole(roleName, permissionProfileIds) {
    if (!roleName) {
      throw new Error('Role name must be specified', { cause: 'undefined' });
    }
    ...
    return this.knex.transaction(async (trx) => {
      const result = await trx('Roles').where({ Name: roleName });
      if (result.length === 0) {
        throw new Error(`Unknown role ${roleName}`, { cause: 'unknown' });
      }
      const roleId = result[0].id;
      await trx('RolePermissionProfiles').where('idRole', roleId).delete();

      const rolePermissionProfiles = permissionProfileIds.map((id) => {
        return { idRole: roleId, idPermissionProfile: id };
      });
      const inserts = await trx('RolePermissionProfiles').insert(rolePermissionProfiles).returning('idPermissionProfile');
      console.log(`Inserted ${inserts.length} RolePermissionProfiles for role ${roleName}`);
      return roleId;
    });
  },
  ...
  async unassignRoleFromUser(roleId, userId) {
    const result = await this.knex('UserRoles').delete({ idUser: userId, idRole: roleId });
  ...
  async getPermissionsForRole(idRole) {
    if (!idRole) {
      throw new Error('Undefined idRole');
    }
    return this.knex.select().table('RolePermissions').where({ idRole });
  },

  async getPermissions() {
    return this.knex.select().table('Permissions');
  },
  ...
  _updateUser(existingUserData, data, updateDataIn) {
    const updateData = updateDataIn;
