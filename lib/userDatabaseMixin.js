...
const UserDatabaseMixin = {
  ...
  async deleteRole(roleId) {
    let result = await this.getPermissionProfilesForRole(roleId);
    if (result.length > 0) {
      throw new Error(`Role with id ${roleId} has PermissionProfiles assigned and can't be deleted`, { cause: 'constrain' });
    }
    result = await this.knex('Roles').where({ id: roleId }).delete();
    if (result.length !== 1) {
      throw new Error(`Role with id ${roleId} does not exist`, { cause: 'unknown' });
    }
  ...
  async checkUserIsAllowed(userId, resource, method) {
    ...
    const queryResult = await this.knex.select(
      ...
      .andWhere('Users.ExpiredAfter', '>', now.toISO())
  ...
  async getUsersPermissions(userId) {
    ...
    const queryResult = await this.knex
      ...
      .where('Users.ExpiredAfter', '>', now.toISO())
      .andWhere({ 'Users.id': userId });
