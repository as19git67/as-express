...
const UserDatabaseMixin = {
  ...
  async createRole(name, permissions) {
    ...
    return this.knex.transaction(async (trx) => {
      ...
      const inserts = await this._addPermissionsToRole(permissions, roleId, trx);
  ...
  async updateRole(name, permissions) {
    ...
    return this.knex.transaction(async (trx) => {
      let result = await trx('Roles').where({ Name: name });
      if (result.length === 0) {
        throw new Error(`Unknown role ${name}`, { cause: 'unknown' });
      }
      const roleId = result[0].id;
      result = await trx('RolePermissions').where('idRole', roleId).delete();
  ...
  async _addPermissionsToRole(permissions, roleId, trx) {
    const checkHash = {};
    ...
    const inserts = await trx('RolePermissions').insert(allRolePermissions).returning('id');
    // await trx.commit();
    return inserts;
  },
  ...
  async setRoleAssignmentsForUser(userId, roleIds) {
    return this.knex.transaction(async (trx) => {
      ...
      const allUserRoles = roleIds.map((roleId) => {
        return { idUser: userId, idRole: roleId };
      });
