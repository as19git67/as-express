...
export default class AsExpress {
  ...
  async init(options) {
    ...
    await this.#initDB(dbSchemas, dbMixins);
  ...
  async #initDB(dbSchemas, dbMixins) {
  ...
  async #initBaseSchema() {
    const exists = await this.#database.isSchemaOK(dbSchema);
    if (exists) {
      ...
      if (initialAdminPassword) {
        try {
          await this.#database.makeSchemaUpToDate(dbSchema);
        } catch (ex) {
