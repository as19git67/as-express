...
export default class AsExpress {
  ...
  async #setAdminRolePermissions() {
    ...
    await this.#database.setPermissionProfileAssignmentsForRole(1, permissionProfileKeys);
  }
  ...
  async #initData() {
    ...
    if (result.length === 0) {
      ...
      if (adminUser && adminUser.length && initialAdminPassword && initialAdminPassword.length) {
        const userId = await this.#database.createUser(adminUser, initialAdminPassword);
        await this.#database.updateUserById(userId, {EmailConfirmed: true, ExpiredAfter: null});
  ...
  #addRoleHandlerToRouter(router, verb, routeConfig) {
    ...
    router[verb](routeConfig.path, (req, res, next) => {
      ...
      this.#isAllowed(userId, resource, action).then((allowed) => {
        ...
        next(new HttpError(500, 'Server error while checking permissions'));
      });
    });
  }
