...
import dbBaseSchema from './schema.js';
...
export default class AsExpress {
  ...
  async init(options) {
    ...
    const dbImporter = [];
    const { dataDirectory, importDatafile } = config;
    ...
    if (dbImporter.length > 0) {
      await this.#importData(dbImporter, path.resolve(dataDirectory, importDatafile));
    }
    this.#initApiRouter();
  ...
  async #initDB(dbSchemas, dbMixins, dropTables) {
    ...
    if (dropTables) {
      await this.#database.dropAllTables([dbBaseSchema].concat(dbSchemas));
    }
    const wasOk = await this.#initBaseSchema();
    if (!wasOk) {
      await this.#initData();
    }
    await this.#setAdminRolePermissions();
    await this.#initAdditionalSchemas(dbSchemas);
  }

  async #setAdminRolePermissions() {
    console.log('Updating admin role with permissions');
    await this.#database.updateRole('admin', this.permissions);
  }
  ...
  async #initBaseSchema() {
    const exists = await this.#database.isSchemaOK(dbBaseSchema);
    if (exists) {
      console.log('Base database schema is ok. Not performing initial config.');
    } else {
      const { initialAdminPassword } = config;
      if (initialAdminPassword) {
        try {
          await this.#database.makeSchemaUpToDate(dbBaseSchema);
        } catch (ex) {
  ...
  addRouter(urlPath, routeConfig) {
    ...
    if (routeConfig && routeConfig.routes) {
      for (const routePath of Object.keys(routeConfig.routes)) {
        for (const verb of ['get', 'post', 'put', 'delete']) {
          if (routeConfig.routes[routePath][verb]) {
            const route = routeConfig.routes[routePath][verb];
            const { bypassAuth } = route;
            if (route.path && route.handler) {
              this.#addCorsHandlerToRouter(router, verb, route, urlPath, bypassAuth, corsOptions);

              if (!bypassAuth) {
                this.#addAuthHandlerToRouter(router, verb, route);
                this.#addRoleHandlerToRouter(router, verb, route, routeConfig);
              }
  ...
  #addRoleHandlerToRouter(router, verb, route, routeConfig) {
    // add the role permissions middleware to check users role has required permissions
    router[verb](route.path, (req, res, next) => {
  ...
  #addAuthHandlerToRouter(router, verb, route) {
    router[verb](route.path, (req, res, next) => {
  ...
  #addCorsHandlerToRouter(router, verb, route, urlPath, bypassAuth, corsOptions) {
    console.log(`adding express router for ${verb} ${urlPath} ${route.path}, bypassAuth: ${bypassAuth}`);
    corsOptions.credentials = !bypassAuth;
    router[verb](route.path, (req, res, next) => {
      console.log('doCORS');
