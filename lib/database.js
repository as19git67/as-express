...
export default class DB {
  ...
  constructor(options) {
    ...
    const {
      dbHost, dbPort, dbName, dbUsername, dbPassword, dbDebug,
    } = config;
    ...
    switch (this.#dbClient) {
      case 'sqlite3': {
        const dbFilename = options.appName ? path.resolve(config.dataDirectory, `${options.appName}.sqlite`) : path.resolve(config.dataDirectory, 'app.sqlite');
  ...
  async isSchemaOK(schema) {
    return this.#existsTable('XXX');
  }

  async makeSchemaUpToDate(schema) {
    const tables = _.map(schema, (tableDef) => tableDef.tableName).reverse();
